...
from comment.utils import (
    ...
    get_profile_content_type,
    is_comment_moderator, is_comment_admin
)
...
def get_comments_count(obj, user):
    return obj.comments.all_comments_by_object(
        obj,
        include_flagged=is_comment_moderator(user)
    ).count()
...
def render_comments(obj, request, oauth=False, comments_per_page=10):
    ...
    """
    context = get_comment_context_data(
        request,
        model_object=obj,
...
def can_delete_comment(comment, user):
    return (
            is_comment_admin(user) or
            (comment.is_flagged and is_comment_moderator(user))
    )
...
def has_reacted(comment, user, reaction):
    ...
    if user.is_authenticated:
        reaction_type = getattr(
            ReactionInstance.ReactionType,
            reaction.upper(),
            None
        )
        if not reaction_type:
            raise template.TemplateSyntaxError(
                'Reaction must be an valid ReactionManager.RelationType. {} is not'.format(
                    reaction
                )
            )
...
def has_flagged(user, comment):
    if user.is_authenticated:
        return FlagInstance.objects.filter(
            user=user,
            flag__comment=comment
        ).exists()
