...
from comment.api.permissions import (
    IsOwnerOrReadOnly, ContentTypePermission, ParentIdPermission,
    FlagEnabledPermission, CanChangeFlaggedCommentState
)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not comment.is_edited and state == flag.RESOLVED:
                return Response(
                    {
                        'error': 'The comment must be edited before '
                                 'resolving the flag'
                    },
                    status=status.HTTP_400_BAD_REQUEST
                )
