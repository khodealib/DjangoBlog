...
def is_comment_admin(user):
    return user.groups.filter(name='comment_admin').exists() or \
        (user.has_perm('comment.delete_flagged_comment') and user.has_perm(
            'comment.delete_comment'))


def is_comment_moderator(user):
    return user.groups.filter(
        name='comment_moderator').exists() or user.has_perm(
...
def get_comment_context_data(request, model_object=None, comments_per_page=0):
    ...
    if not comments_per_page:
        comments_per_page = request.POST.get(
            'comments_per_page') or request.GET.get('comments_per_page')
    ...
    if not login_url:
        raise ImproperlyConfigured(
            'Comment App: LOGIN_URL is not in the settings')
